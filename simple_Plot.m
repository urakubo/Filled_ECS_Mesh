...
function simpleplot
	...
	Dendrite_big = [2, 11, 68, 64, 29, 12, 28];
	...
	UnID     = 1:SegNUM;
	UnID     = setdiff(UnID,Axon);
	UnID     = setdiff(UnID,Dendrite);
	UnID     = setdiff(UnID,Glia);
	numel(UnID)
	DA_ID = find(rand(size(Axon)) > 0.8);

	xlabel('(\mum)');
	ylabel('(\mum)');
	zlabel('(\mum)');
	hold on;
	for i = 1:SegNUM; % Dendrite Axon Glia UnID Axon(DA_ID) [1:SegNUM];
		plotmesh(Obj{i,1}*p.xp /1000, Obj{i,2},'facecolor',col(i,:)); 
	end;
	view(p.az,p.el);

	saveas(gca, 'All.png');% Dendrite Axon Glia UnID
